import { NextResponse } from "next/server";
import { createClient } from "@supabase/supabase-js";

export async function POST(req: Request) {
  try {
    const body = await req.json();
    const { name, role, photo } = body as {
      name: string;
      role?: string;
      photo?: string;
    };
    if (!name)
      return NextResponse.json(
        { success: false, error: { message: "name required" } },
        { status: 400 },
      );

    const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || "";
    const serviceRoleKey =
      process.env.SUPABASE_SERVICE_ROLE_KEY ||
      process.env.SUPABASE_ROLE_KEY ||
      process.env.NEXT_PUBLIC_SUPABASE_ROLE_KEY ||
      process.env.NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY ||
      "";
    if (!supabaseUrl || !serviceRoleKey) {
      return NextResponse.json(
        {
          success: false,
          error: {
            message: "Missing Supabase service role key or URL on server",
          },
        },
        { status: 500 },
      );
    }

    const serverClient = createClient(supabaseUrl, serviceRoleKey, {
      auth: { persistSession: false },
    });

    // attempt to set position to max(position) + 1 when the column exists
    let positionToSet: number | undefined = undefined;
    try {
      const posRes = await serverClient
        .from("members")
        .select("position")
        .order("position", { ascending: false })
        .limit(1);
      if (
        posRes.data &&
        posRes.data.length > 0 &&
        typeof posRes.data[0].position === "number"
      ) {
        positionToSet = (posRes.data[0].position as number) + 1;
      }
    } catch (e) {
      // ignore if column doesn't exist
    }

    const insertPayload: any = { name, role: role || "", photo: photo || "" };
    if (typeof positionToSet === "number")
      insertPayload.position = positionToSet;

    const { data, error } = await serverClient
      .from("members")
      .insert([insertPayload]);
    if (error)
      return NextResponse.json(
        { success: false, error: { message: error.message || String(error) } },
        { status: 500 },
      );
    return NextResponse.json({ success: true, data });
  } catch (e: any) {
    return NextResponse.json(
      { success: false, error: { message: e?.message ?? String(e) } },
      { status: 500 },
    );
  }
}
